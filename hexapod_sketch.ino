...
const char *ssid = "hexapod";//APSSID;
const char *password = "hexapod_1234";//APPSK;
//AsyncUDP udp_socket;
WiFiUDP udp;
...
void setup()
{
  ...
  */
  ...
  //boot_up_motion(lut_config_length, lut_config);
  //moveFrontLeft(350);
  //  posture_calibration();
}
...
void loop()
{
  ...
  if (packetSize)
  {
    ...
    if (len > 0)
    {
      inPacket[len] = '\0'; // Null-terminate the string
    }

    Serial.printf("Received: %s\n", inPacket);

    //
    if (strcmp(inPacket, "standby") == 0)
    {
      next_motion = MotionMode::Mode_Standby;
    }
    else if (strcmp(inPacket, "calibrate") == 0)
    {
      posture_calibration();
    }
    else if (strcmp(inPacket, "mfl100") == 0)
    {
      moveFrontLeft(100);
    }
    else if (strcmp(inPacket, "mfl200") == 0)
    {
      moveFrontLeft(200);
    }
    else if (strcmp(inPacket, "walk0") == 0)
    {
      next_motion = MotionMode::Mode_Walk_0;
      //Serial.println("Received walk0!");
    }
    else if (strcmp(inPacket, "walk180") == 0)
    {
      next_motion = MotionMode::Mode_Walk_180;
    }
    else if (strcmp(inPacket, "walkr45") == 0)
    {
      next_motion = MotionMode::Mode_Walk_R45;
    }
    else if (strcmp(inPacket, "walkr90") == 0)
    {
      next_motion = MotionMode::Mode_Walk_R90;
    }
    else if (strcmp(inPacket, "walkr135") == 0)
    {
      next_motion = MotionMode::Mode_Walk_R135;
    }
    else if (strcmp(inPacket, "walkl45") == 0)
    {
      next_motion = MotionMode::Mode_Walk_L45;
    }
    else if (strcmp(inPacket, "walkl90") == 0)
    {
      next_motion = MotionMode::Mode_Walk_L90;
    }
    else if (strcmp(inPacket, "walkl135") == 0)
    {
      next_motion = MotionMode::Mode_Walk_L135;
    }
    else if (strcmp(inPacket, "fastforward") == 0)
    {
      next_motion = MotionMode::Mode_Fast_Forward;
    }
    else if (strcmp(inPacket, "fastbackward") == 0)
    {
      next_motion = MotionMode::Mode_Fast_Backward;
    }
    else if (strcmp(inPacket, "turnleft") == 0)
    {
      next_motion = MotionMode::Mode_Turn_Left;
    }
    else if (strcmp(inPacket, "turnright") == 0)
    {
      next_motion = MotionMode::Mode_Turn_Right;
    }
    else if (strcmp(inPacket, "climbforward") == 0)
    {
      next_motion = MotionMode::Mode_Climb_Forward;
    }
    else if (strcmp(inPacket, "climbbackward") == 0)
    {
      next_motion = MotionMode::Mode_Climb_Backward;
    }
    else if (strcmp(inPacket, "rotatex") == 0)
    {
      next_motion = MotionMode::Mode_Rotate_X;
    }
    else if (strcmp(inPacket, "rotatey") == 0)
    {
      next_motion = MotionMode::Mode_Rotate_Y;
    }
    else if (strcmp(inPacket, "rotatez") == 0)
    {
      next_motion = MotionMode::Mode_Rotate_Z;
    }
    else if (strcmp(inPacket, "twist") == 0)
    {
      next_motion = MotionMode::Mode_Twist;
    }
  //Executing the motion VV :)
  }
...
{
  for (int i = 0; i < 3; i++)
    left_pwm.setPWM(left_legs[0][i], 0, move);
}
