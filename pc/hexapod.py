...
class MyApp(QtWidgets.QMainWindow):
	...
	def init_ui(self):
		...
		udp_client_ip = self.config.get('UDP_Client_IP', '192.168.4.1')
		udp_client_port = self.config.get('UDP_Client_Port', '1234')
		self.ui.lineEdit_UdpClientTargetIP.setText(udp_client_ip)
		self.ui.lineEdit_UdpClientTargetPort.setText(udp_client_port)
	...
	def on_standby_button_clicked(self):
		if self.is_udp_connected:
			self.udp_client.send(self.CMD_STANDBY)
	...
	def on_forward_button_clicked(self):
		if self.is_udp_connected:
			self.udp_client.send(self.CMD_WALK_0)
	...
	def on_udp_client_connect_button_clicked(self):
		if self.ui.buttonUdpConnect.text() == 'Connect':
			...
			self.udp_client.message.connect(self.on_udp_client_message_ready)

			self.udp_client.moveToThread(self.udp_client_thread)
